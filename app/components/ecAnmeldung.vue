...
v-form(v-if="(force || (!disabled && !countdown)) && !success")
  v-radio-group(v-model="data.geschlecht" required label="Geschlecht" @update:model-value="geschlechtEvent" :error-messages="geschlechtErrors")
  ...
  div(v-if="hatErlaubnisse && under18")
    ...
    p(class="pt-3") Mein
      ...
      span(v-else)  Kind
      |  darf...
    v-checkbox(density="compact" class="mt-0" label="Bootfahren" v-model="data.bootfahren" v-if="hatErlaubnisBoot")
    v-checkbox(density="compact" class="mt-0" label="Radfahren" v-model="data.fahrrad" v-if="hatErlaubnisFahrrad")
    v-checkbox(density="compact" class="mt-0" label="Klettern" v-model="data.klettern" v-if="hatErlaubnisKlettern")
    v-checkbox(density="compact" class="mt-0" label="Sich in Gruppen von mindestens 3 Personen entfernen" v-model="data.sichEntfernen" v-if="hatErlaubnisSichEntfernen")
    div
...
div(v-else-if="success")
  ...
  v-alert(type="warning" rounded="0" :icon="mdiInformation").text-secondary
